...
class HDFGroup {
  HDFGroup(boost::shared_ptr<typename HDFImpl::group_type> group)
      : group(group) {}
 ...
 protected:
   ...
   * within another file
   */
  void initFileGroup(typename HDFImpl::file_handle_type &file,
                     const std::string &path = std::string("/")) {
    group = HDFImpl::openGroup(file, path, false);  // pass true to create here
                                                    // for the case where we are
                                                    // mounting a file in a file
                                                    // where the mount point
                                                    // doesn't yet exist - but
                                                    // only if path != "/"
  }
#ifdef H5_HAVE_PARALLEL
  void initFileGroup(typename HDFImpl::parallel_file_handle_type &file,
                     const std::string &path = std::string("/")) {
    group = HDFImpl::openGroup(file, path, false);  // pass true to create here
                                                    // for the case where we are
                                                    // mounting a file in a file
 ...
 public:
   ...
   */
  boost::shared_ptr<HDFGroup<HDFImpl> > openGroup(const std::string &path,
                                                  bool create = false) {
    try {
      return boost::shared_ptr<HDFGroup<HDFImpl> >(
          new HDFGroup<HDFImpl>(HDFImpl::openGroup(*group, path, create)));
    }
   ...
   * Create a new group
   */
  boost::shared_ptr<HDFGroup<HDFImpl> > createGroup(const std::string &path) {
    return boost::shared_ptr<HDFGroup<HDFImpl> >(
        new HDFGroup<HDFImpl>(HDFImpl::createGroup(*group, path)));
  }

  /**
   * Create a new external link
   */
  boost::shared_ptr<HDFGroup<HDFImpl> > createExternalLink(
      const std::string &externalFile, const std::string &externalPath,
      const std::string &path) {
    return boost::shared_ptr<HDFGroup<HDFImpl> >(new HDFGroup<HDFImpl>(
        HDFImpl::createExternalLink(*group, externalFile, externalPath, path)));
  }
  ...
  boost::shared_ptr<HDFAttribute<HDFImpl> > createAttribute(
      const std::string &name, std::vector<hsize_t> dims) {
    return boost::shared_ptr<HDFAttribute<HDFImpl> >(new HDFAttribute<HDFImpl>(
   ...
   */
  boost::shared_ptr<HDFAttribute<HDFImpl> > openAttribute(
      const std::string &name) {
    return boost::shared_ptr<HDFAttribute<HDFImpl> >(
        new HDFAttribute<HDFImpl>(HDFImpl::openAttribute(*group, name)));
  }

  /**
   * Open an existing dataset
   */
  boost::shared_ptr<HDFDataSet<HDFImpl> > openDataset(const std::string &path) {
    return boost::shared_ptr<HDFDataSet<HDFImpl> >(
        new HDFDataSet<HDFImpl>(HDFImpl::openDataSet(*group, path)));
  }
  ...
  boost::shared_ptr<HDFDataSet<HDFImpl> > createDataset(
      const std::string &path, const Slab<order, HDFImpl> &dims) {
    try {
      return boost::shared_ptr<HDFDataSet<HDFImpl> >(new HDFDataSet<HDFImpl>(
  ...
  template <typename Type>
  boost::shared_ptr<HDFAttribute<HDFImpl> > writeAttribute(
      const std::string &path, const Type &data) {
   ...
   */
  template <typename Type>
  boost::shared_ptr<HDFDataSet<HDFImpl> > writeDataset(
      const std::string &path, const std::vector<Type> &data) {
