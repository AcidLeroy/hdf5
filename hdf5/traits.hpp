...
struct TypeCreatorHelper {
  TypeCreatorHelper(hid_t &type) : type(type), offset(0) {
    ...
    boost::mpl::for_each<typename boost::fusion::result_of::as_vector<T>::type>(
        boost::ref(*this));
...
struct TypeCreatorHelper<T, boost::true_type> {
  TypeCreatorHelper(hid_t &type) {
    type = data_type_traits<
        typename boost::remove_cv<T>::type>::homogeneous_type();
  }
};
...
struct DataTypeCreator {
  ...
  DataTypeCreator(wrapper<T>) {
    TypeCreatorHelper<T, typename data_type_traits<typename boost::remove_cv<
                             T>::type>::is_homogeneous> t(type);
...
class data_type_traits {
 public:
  ...
  typedef typename boost::is_same<
      ...
      boost::mpl::int_<boost::mpl::count<
          vec_type,
          typename boost::mpl::at_c<vec_type, 0>::type>::type::value> >::type
      is_homogeneous;

  static hid_t homogeneous_type() {
    return H5Tcopy(data_type_traits<
        typename boost::mpl::at_c<vec_type, 0>::type>::value());
  }

  static hsize_t dimsize() {
    if (is_homogeneous::value)
      return boost::mpl::count<
          vec_type, typename boost::mpl::at_c<vec_type, 0>::type>::type::value;
    else
      return 1;
  }
...
class HDF5ParallelFileHolder : boost::noncopyable {
 public:
  ...
  ~HDF5ParallelFileHolder() { H5Fclose(file); }
...
class HDF5DataSpace {
 public:
  ...
  HDF5DataSpace(const HDF5DataSpace &orig, const std::vector<hsize_t> &offset,
  ...
  template <typename Type>
  static boost::shared_ptr<HDF5DataSpace> selectSubset(
      const HDF5DataSpace &orig, const std::vector<Type> &mapping) {
    ...
    for (typename std::vector<Type>::const_iterator i = mapping.begin();
      ...
      for (hsize_t l = 0; l < dim2size; ++l) {
        *j = *i;
        ...
        for (hsize_t k = 1; k < dataspaceDims; ++k, ++j) *j = l;
      }
...
class HDF5DataSet : boost::noncopyable {
 public:
  ...
  template <class Parent>
  HDF5DataSet(Parent &p, const std::string name, const HDF5DataType &datatype,
...
//       H5Pset_chunk(cparms, 2, chunk_dims);
#if H5_VERS_MINOR >= 8
    dataset = H5Dcreate(p.hid(), name.c_str(), datatype.hid(), dataspace.hid(),
  ...
  boost::shared_ptr<HDF5DataSpace> getDataSpace() const { return space; }

  template <typename Type>
  boost::shared_ptr<HDF5DataSet> selectSubset(
      const std::vector<Type> &mapping) {
...
class HDF5Group : boost::noncopyable {
 public:
  ...
  template <class Parent>
  HDF5Group(Parent &p, const std::string &path,
            bool create) throw(GroupNotFound) {
...
// Group didn't exist and we've asked to create the group
      ...
      group = H5Gcreate(p.hid(), path.c_str(), 0);
#endif
      if (group < 0) throw;  // Error creating group
    } else {
      throw GroupNotFound();
    }
  }
   ...
   */
  template <class Parent>
  HDF5Group(Parent &p, const std::string &externalFile,
            const std::string &externalPath, const std::string &path, Create) {
#if H5_VERS_MINOR >= 8
...
#else
    attribute =
        H5Acreate(p.hid(), name.c_str(), type.hid(), space.hid(), H5P_DEFAULT);
#endif
    check_errors();
  }
...
class HDF5Traits {
 public:
  ...
  static boost::shared_ptr<file_handle_type> open(const std::string &path,
                                                  bool truncate) {
    if (truncate) {
      detail::Create c;
...
#ifdef H5_HAVE_PARALLEL
  static boost::shared_ptr<parallel_file_handle_type> parallel_open(
    ...
    return boost::shared_ptr<parallel_file_handle_type>(
        new parallel_file_handle_type(path));
  }
#endif
  template <typename FileHandle>
  static boost::shared_ptr<group_type> openGroup(FileHandle &f,
                                                 const std::string &path,
                                                 bool create) {
    return boost::shared_ptr<group_type>(new group_type(f, path, create));
  }

  static boost::shared_ptr<group_type> openGroup(group_type &f,
                                                 const std::string &path,
                                                 bool create) {
    return boost::shared_ptr<group_type>(new group_type(f, path, create));
  }
  ...
  template <typename FileHandle>
  static boost::shared_ptr<group_type> createGroup(FileHandle &f,
                                                   const std::string &path) {
    return boost::shared_ptr<group_type>(
        new group_type(f, path, detail::Create()));
  }

  static boost::shared_ptr<group_type> createGroup(group_type &f,
                                                   const std::string &path) {
    return boost::shared_ptr<group_type>(
        new group_type(f, path, detail::Create()));
  }

  static boost::shared_ptr<group_type> createExternalLink(
      group_type &f, const std::string &externalFile,
      const std::string &externalPath, const std::string &path) {
    return boost::shared_ptr<group_type>(
        new group_type(f, externalFile, externalPath, path, detail::Create()));
  }

  template <typename FileHandle>
  static boost::shared_ptr<dataset_type> openDataSet(FileHandle &f,
                                                     const std::string &path) {
    return boost::shared_ptr<dataset_type>(new dataset_type(f, path));
  }

  static boost::shared_ptr<dataset_type> openDataSet(group_type &f,
                                                     const std::string &path) {
    return boost::shared_ptr<dataset_type>(new dataset_type(f, path));
  }

  template <class Type>
  static boost::shared_ptr<attribute_type> createAttribute(
      group_type &g, const std::string &name, const std::vector<hsize_t> &dims,
      const std::vector<hsize_t> &maxdims) {
  ...
  template <class Type>
  static boost::shared_ptr<attribute_type> createAttribute(
      dataset_type &g, const std::string &name,
      const std::vector<hsize_t> &dims, const std::vector<hsize_t> &maxdims) {
    return boost::shared_ptr<attribute_type>(
        new attribute_type(g, name, dims, maxdims, Type()));
  }

  static boost::shared_ptr<attribute_type> openAttribute(
      group_type &g, const std::string &name) {
  ...
  static boost::shared_ptr<dataset_type> createDataSet(
      FileHandle &f, const std::string &path,
  ...
  template <typename Type>
  static boost::shared_ptr<dataset_type> createDataSet(
      group_type &f, const std::string &path,
      const detail::HDF5DataSpace &space) {
  ...
  template <typename Type>
  static void write_attribute(const attribute_type &attribute,
                              const Type &data) {
    detail::wrapper<Type> t;
    detail::HDF5DataType datatype(t);
  ...
  */
  template <typename Type>
  static void write_dataset(const dataset_type &dataset,
                            const std::vector<Type> &data) {
    ...
    /*
            if(d.size()==1){
              ...
              herr_t status = H5Dwrite(dataset.hid(), memdatatype.hid(),
       memorySpace.hid(), H5S_ALL,
                  H5P_DEFAULT, &data[0]);
              assert(status == 0);
            }
            else
...
#ifdef H5_HAVE_PARALLEL
  template <typename Type>
  static void write_parallel_dataset(const dataset_type &dataset,
                                     const std::vector<Type> &data) {
    hid_t plist_id;
...
#endif
  template <typename Type>
  static void write_dataset(const dataset_type &dataset, const Type *data,
                            const detail::HDF5DataSpace &memorySpace) {
    detail::wrapper<Type> t;
    detail::HDF5DataType memdatatype(t);
    boost::shared_ptr<detail::HDF5DataSpace> fileSpace = dataset.getDataSpace();

    H5Dwrite(dataset.hid(), memdatatype.hid(), memorySpace.hid(),
  ...
  template <typename Type>
  static void read_dataset(const dataset_type &dataset,
    ...
    detail::wrapper<Type> t;
    detail::HDF5DataType datatype(t);
    H5Dread(dataset.hid(), datatype.hid(), memorySpace.hid(), fileSpace->hid(),
            H5P_DEFAULT, &data[0]);
  }
  ...
  static void read_dataset(const dataset_type &dataset, Type &data) {
    ...
    detail::wrapper<Type> t;
    detail::HDF5DataType datatype(t);
    H5Dread(dataset.hid(), datatype.hid(), fileSpace->hid(), fileSpace->hid(),
            H5P_DEFAULT, data);
  }
  ...
  static std::vector<hsize_t> getDims(const std::vector<Type> &t) {
    // if( detail::data_type_traits<Type>::is_homogeneous::value)
    {
     ...
     detail::data_type_traits<Type>::num_type());
       dims[0] = t.size();
       return dims;
     }*/
  }
