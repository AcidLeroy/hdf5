...
TEST_F(Hdf5Test, CreateDataset) {
	...
	boost::shared_ptr<hdf::HDFDataSet<> > datasetint = file.createDataset<int>(
			"/test", filespace);
	boost::shared_ptr<hdf::HDFDataSet<> > datasetfloat = file.createDataset<
			float>("/test-float", filespace);
	boost::shared_ptr<hdf::HDFDataSet<> > datasetdouble = file.createDataset<
			double>("/test-double", filespace);
}
...
TEST_F(Hdf5Test, OpenDataset) {
	hdf::HDFFile<> file(file_name_, hdf::HDFFile<>::truncate);
	{
		...
		dims[1] = 5;
		hdf::Slab<2> filespace(dims);
		boost::shared_ptr<hdf::HDFDataSet<> > datasetint = file.createDataset<
				int>("/test", filespace);
	}
...
TEST_F(Hdf5Test, OpenGroup) {
	...
	{
		boost::shared_ptr<hdf::HDFGroup<> > group = file.createGroup(
				"/test-group");
	}
...
TEST_F(Hdf5Test, WriteAttribute) {
	...
	group->createAttribute<int>("attr", dims);
	boost::shared_ptr<hdf::HDFAttribute<> > attr = group->openAttribute(
			std::string("attr"));
	attr->writeData(10);
	int val;
	attr->readData(val);
